...
export function DatabaseProvider ({ children }) {
  ...
  const itemsToDonateForm = async () => {
    if (checked.length > 1 || handleButton.slice(1, -1).length > 1 || !inputNameCity || inputNameCity === '— wybierz —' || !buttonList || buttonList.length > 1 || nameOrg.slice(1, -1).length > 1 || !nameOrg || !clientAdress || clientAdress.slice(1, -1).length > 1 || !clientCity || clientCity.slice(1, -1).length > 1 || !clientPostNumber || clientPostNumber.slice(1, -1).length > 1 || !clientPhoneNumber || !dataDeliver || !pickupTime || !noteToTheCourier) {
      setInputError('Please fill in all the fields coreectly')
      return
    }
    const { data, error } = await supabase
