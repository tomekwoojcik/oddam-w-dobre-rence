...
export function DatabaseProvider ({ children }) {
  ...
  const itemsToDonateForm = async () => {
    if (checked.length > 1 || handleButton.slice(1, -1).length > 1 || !inputNameCity || !buttonList || !nameOrg || !clientAdress || !clientCity || !clientPostNumber || !clientPhoneNumber || !dataDeliver || !pickupTime || !noteToTheCourier) {
      setInputError('Please fill in all the fields coreectly')
      return
    }
    const { data, error } = await supabase
