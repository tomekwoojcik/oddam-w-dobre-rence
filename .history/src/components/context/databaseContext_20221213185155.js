...
export function DatabaseProvider({ children }) {
  ...
  const [arrayPaginate, setArrayPaginate] = useState(database.slice(0, 3))
  ...
  const displayItems = database.slice(pagesVisited, pagesVisited + itemsPerPages)
  const handlePagination = () => {

  }

  return (
    <DatabaseContext.Provider value={{ database, fetchError, getValue, setUserName, setUserEmail, setMessage, userEmail, userName, message, sentDataContactUs, inputError, arrayPaginate }}>{children}</DatabaseContext.Provider>
  )
}
