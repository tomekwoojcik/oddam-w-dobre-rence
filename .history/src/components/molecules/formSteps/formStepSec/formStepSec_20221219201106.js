...
const FormStepSec = () => {
  const { inputName, listFormStepHandle, arrowRotare, arrowValueList, handleButton, setHandleButton } = useContext(DatabaseContext)
  return (
    <div className='formStepOne'>
      ...
      <div className='rowEnd'>
        ...
        {arrowRotare !== true ? null : <div className='inputNameListValue'>{arrowValueList.map((el) => { return <button value={handleButton} onClick = {(e) => { setHandleButton(e.target.value) }} className='listButton' key={el}>{el}</button> })}</div>}
      </div>
    </div>
  )
}
