...
const WhoDoYouWantToHelpButtonList = () => {
    ...
    return (
        <ul className='whoDoYouWantToHelpButtonList'>
            {whoDoYouWantToHelpArray.map((el) => { return (<li className='whoDoYouWantListItem' key={el}><button onClick={() => { handleWhoDoYouWantToHelpButtonList() }} className = 'whoDoYouWantButton' > { el }</button></li >)
})}
        </ul >
  )
}
