  ...
  return (
        <ul className='whoDoYouWantToHelpButtonList'>
            {whoDoYouWantToHelpArray.map((el) => {
              return (<li className='whoDoYouWantListItem' key={el}><button value={el} style={{ backgroundColor: backgroundButton ? '#FAD648' : '' }} onClick={(e) => { handleWhoDoYouWantToHelpButtonList(e) }} className='whoDoYouWantButton' > {el}</button></li >)
            })}
        </ul >
  )
}
